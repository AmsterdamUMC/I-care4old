...
X_orig_train = pd.read_pickle(data_save_dir+'data_train_ach_new_users_hc.pki')
X_orig_test = pd.read_pickle(data_save_dir+'data_test_ach_new_users_hc.pki')
X_train = pd.read_pickle(data_save_dir+'data_train_norm_ach_new_users_hc.pki')
X_test = pd.read_pickle(data_save_dir+'data_test_norm_ach_new_users_hc.pki')
...
'unsteady_gait_or_falls',
'atc_diuretics',
'atc_cardiac_therapy',
'anticoagulant_temp',
'atc_antithrombotic_agents',
'ADLH_4',
'CPS_3',
'pain_scale',
'IKA',
...
y_train_all = X_orig_train['hospital_12months'].astype(float)
x_train_all = X_orig_train
y_test = X_orig_test['hospital_12months'].astype(float)
x_test = X_orig_test
...
file_name = '/data/hyperparameters_anticholinergic_hospitalization'
...
ate_train = np.mean(treatment_effects_train)
df_omnibus = pd.DataFrame()
df_omnibus['y'] = np.round(treatment_effects,5)
df_omnibus['x1'] = np.round(ate_train,5)
df_omnibus['x2'] = np.round(treatment_effects,5) - df_omnibus['x1']


#%%
from scipy.stats import pearsonr
corr, _ = pearsonr(df_ITE_values['DML'], df_ITE_values['DR'])
print (corr)
...
for model_nro in range(len(models)):
    ...
    for train_idx, test_idx in kf.split(X_train_all):
        ...
        # Plot uplift curve
        if (model_nro == 3): # Piirretään käyrät valitulle mallille
            uplift_disp=plot_uplift_curve(test['y'], treatment_effects, test['T'], perfect=False);
...
df_ITE_results.to_csv(presentation_dir + '/data/df_models_random_performance.csv')
