...
X_orig_train = pd.read_pickle(data_save_dir+'data_hist_train_psychotropic_disc_ltc.pki')
X_orig_test = pd.read_pickle(data_save_dir+'data_hist_test_psychotropic_disc_ltc.pki')
X_train = pd.read_pickle(data_save_dir+'data_hist_train_norm_psychotropic_disc_ltc.pki')
X_test = pd.read_pickle(data_save_dir+'data_hist_test_norm_psychotropic_disc_ltc.pki')
...
outcome = 'hilmo_hospital_visits_12months_target'

#%%
# DATA-DRIVEN FEATURE IMPORTANCE VALUES

variables_sel_manual, variables_sel_manual_names = select_variables_ps(variables = 'ate_antipsychotics_ltc')
variables_sel_manual.remove('HT_NRO')
variables_sel_manual_names.remove('HT_NRO')

test_set_size = 1000 # Test set size
...
'psychotic_symptoms',
'J1_N',
'acute_episode_monitoring_acute_condition',
'hospital_er_last6months',
'Chemotherapy_or_end_stage_disease',
'K1_D',
'any_restrains_used',
'Physician_visits_doctors_orders_changed_abnormal_labs',
]
...
'Delusions OR halucinations',
'Unsteady gait',
'Flare-up or monitoring of acute condition',
'Hospital or ER visits (last 6 months)',
'Chemotherapy or end stage disease',
'No mouth problems',
'Any restrains used',
'Physician visits, doctors orders changed or abnormal labs',
]
...
sns.distplot(x0, label='Users')
sns.distplot(x1, label='Prev users')
plt.xlim(0, 1)
plt.ylabel('Density')
...
#%%
# SELECT OUTCOME (HOSPITALIZATION)

# Normal data (no normaliztion)
y_train_all = X_orig_train['hilmo_hospital_visits_12months_target'].astype(float)
x_train_all = X_orig_train
y_test = X_orig_test['hilmo_hospital_visits_12months_target'].astype(float)
x_test = X_orig_test

# Normalized data
#y_train_all = X_train['hospital_12months'].astype(float)
#x_train_all = X_train
#y_test = X_test['hospital_12months'].astype(float)
#x_test = X_test
...
#%%
number_of_best_features = len(confounders)

ate_results_train = []
ate_results = []
cate_results = []
cate_results_train = []
feature_importances_cf = []
df_cf_feature_importances = pd.DataFrame()
auuc_all = []


for no_confounders in range (len(confounders)):
    auuc = []
    for aa in range (10):

        x_train, x_val, y_train, y_val = train_test_split(x_train_all, y_train_all, test_size=0.2, random_state=seed+aa)

        variables_model = confounders[:no_confounders+1]

        np.random.seed(123)
       
        est = DML(model_y=RandomForestRegressor(max_depth=4,n_estimators=12),
                  model_t=RandomForestRegressor(max_depth=4,n_estimators=12),
                  #model_final=RandomForestRegressor(max_depth=2,n_estimators=8))
                  model_final=LinearRegression())
                  #model_final=LassoCV(cv=3))
        
        est.fit(y_train, x_train['T'], X=x_train[variables_model], W=None)


        treatment_effects_train = est.effect(x_train[variables_model], T0=0, T1=1)
        cate_results_train.append(treatment_effects_train)

        treatment_effects = est.effect(x_val[variables_model], T0=0, T1=1)
        cate_results.append(treatment_effects)

        print ('ATE:')
        print (str(np.round(np.mean(treatment_effects),3)))

        ate = est.ate(x_val[variables_model], T0=0, T1=1)
        ate_results.append(np.round(ate,3))

        from sklift.metrics import uplift_auc_score
        # AUUQ = area under uplift curve
        auuc.append(uplift_auc_score(y_val, (treatment_effects), x_val['T']))
        print(f"Uplift auc score on validation data: {auuc[aa]:.4f}")
    auuc_all.append(auuc)




#%%
for kk in range(len(auuc_all)):
    auuc_mean = np.round(np.mean(auuc_all[kk]),3)
    ci_low = np.round(st.t.interval(alpha=0.95, df=len(auuc_all[kk])-1, loc=np.mean(auuc_all[kk]), scale=st.sem(auuc_all[kk]))[0],3)
    ci_high = np.round(st.t.interval(alpha=0.95, df=len(auuc_all[kk])-1, loc=np.mean(auuc_all[kk]), scale=st.sem(auuc_all[kk]))[1],3)
...
for model_nro in range(len(models)):
    ...
    for train_idx, test_idx in kf.split(X_train_all):
        ...
        # Plot uplift curve
        if (model_nro==3): # talletaan valitun mallin käyrät
...
for k in range (len(auuc_models)):
    model_ate.append(str(np.round(np.mean(ate_all_models[k]),5)) + ' (' + str(np.round(np.min(ate_all_models[k]),4)) + ',' + str(np.round(np.max(ate_all_models[k]),4))+')')
    model_auuc.append(str(np.round(np.mean(auuc_models[k]),5)) + ' (' + str(np.round(np.min(auuc_models[k]),4)) + ',' + str(np.round(np.max(auuc_models[k]),4))+')')
...
df_model_selection['ATE, mean (min,max)'] = model_ate
df_model_selection['AUUC, mean (min,max)'] = model_auuc
df_model_selection['C-for-benefit, mean (min,max)'] = model_c_for_benefit
#df_model_selection['Match error, mean (min,max)'] = match_error

file_name = '/data/hyperparameters_antipsychotics_disc_hospitalization'
...
length = len(treatment_effects)
x=[]
x.append(pd.DataFrame(treatment_effects[0:]).sort_values(by=[0], ascending=False).reset_index(drop=True)[:int(length/5)].mean()[0])
x.append(pd.DataFrame(treatment_effects[0:]).sort_values(by=[0], ascending=False).reset_index(drop=True)[1*int(length/5):2*int(length/5)].mean()[0])
x.append(pd.DataFrame(treatment_effects[0:]).sort_values(by=[0], ascending=False).reset_index(drop=True)[2*int(length/5):3*int(length/5)].mean()[0])
x.append(pd.DataFrame(treatment_effects[0:]).sort_values(by=[0], ascending=False).reset_index(drop=True)[3*int(length/5):4*int(length/5)].mean()[0])
x.append(pd.DataFrame(treatment_effects[0:]).sort_values(by=[0], ascending=False).reset_index(drop=True)[4*int(length/5):].mean()[0])
...
plt.ylabel('Hospitalization rate', fontsize=14)
...
ax.set_ylabel('Difference in absolute risk \n of hospitalization within 12 months', fontsize = 36)
...
for bb in range (len(confounders)):
    list_temp = []
    for aa in range(len(shap_values[outcome]['T_True'].values)):
        list_temp.append(list(shap_values[outcome]['T_True'].values[aa])[bb])
...
plt.title('DML')
shap.summary_plot(shap_values[outcome]['T_True'], max_display = 10, feature_names = names)
...
for no_confounders in range (len(confounders)):
    ...
    for aa in range (10):
        ...
        x_train, x_val, y_train, y_val = train_test_split(x_train_all, y_train_all, test_size=0.2, random_state=11+aa)

        variables_model = confounders[:no_confounders+1]

        est = DRLearner(model_propensity=RandomForestClassifier(max_depth=4,n_estimators=16),               
                    model_regression=RandomForestRegressor(max_depth=4,n_estimators=16),
                    model_final=LinearRegression(),
                    #model_final=LassoCV(cv=3),
                    featurizer=None)
        ...
        print ('ATE: ' + str(np.round(np.mean(treatment_effects),3)))

        ate = est.ate(x_val[variables_model], T0=0, T1=1)
        ate_results.append(np.round(ate,3))

        auuc.append(uplift_auc_score(y_val, (treatment_effects), x_val['T']))
        print(f"Uplift auc score on full data: {auuc[aa]:.4f}")
    auuc_all.append(auuc)
...
for model_nro in range(len(models)):
    ...
    for train_idx, test_idx in kf.split(X_train_all):
        ...
        # Plot uplift curve
        if (model_nro == 7): # Piirretään käyrät valitulle mallille
            uplift_disp=plot_uplift_curve(test['y'], treatment_effects, test['T'], perfect=False);
...
ax.set_ylabel('Difference in absolute risk \n of hospitalization within 12 months', fontsize = 28)
ax.set_xlabel('Patients (sorted)', fontsize = 36)
...
#%%
# Concordance-statistics for benefit (PS MATCHING)
x_test['y'] = y_test
X_t = x_test[x_test['T'] == True].reset_index(drop=True)
X_c = x_test[x_test['T'] == False].reset_index(drop=True)
point_t = est.effect(X_t[variables_model], T0=0, T1=1)
point_c = est.effect(X_c[variables_model], T0=0, T1=1)
X_t['te'] = point_t
X_c['te'] = point_c
...
for no_confounders in range (len(confounders)):
    auuc = []
    for aa in range (10):
        x_train, x_val, y_train, y_val = train_test_split(x_train_all, y_train_all, test_size=0.3, random_state=11+aa)

        variables_model = confounders[:no_confounders+1]

        np.random.seed(123)
        est = XLearner(models=[RandomForestRegressor(max_depth=4,n_estimators=16, random_state=0), 
                               RandomForestRegressor(max_depth=4,n_estimators=16, random_state=0)], 
                               cate_models=None, propensity_model=LogisticRegression())

        est.fit(y_train, x_train['T'], X=x_train[variables_model])
...
plt.figure(facecolor=(1, 1, 1))
plt.title('X learner (Hospitalization 12 months)')
shap.summary_plot(shap_values[outcome]['T_True'], max_display = 10, feature_names = names)
...
no_confounders = len(confounders)
...
for aa in range (5):
    ...
    est_cf = CausalForestDML(model_y = RandomForestRegressor(), model_t= RandomForestClassifier(), 
                                discrete_treatment = True, 
                                cv= 3, 
                                max_depth = 3, 
                                n_estimators = 12, 
                                max_features = 'sqrt', inference = True, random_state = 123, verbose = 0)
    est_cf = est_cf.fit(Y = y_train, T = x_train['T'], X = x_train[variables_model], W = x_train[variables_model], cache_values=True)
    treatment_effects_train = est_cf.effect(x_train[variables_model], T0=0, T1=1)
    cate_results_train.append(treatment_effects_train)

    treatment_effects = est_cf.effect(x_val[variables_model], T0=0, T1=1)
    cate_results.append(treatment_effects)
...
#fig.savefig(presentation_dir + 'imgs/hospitalization/FIG_uplift_by_percentile_cf.eps',format='eps')
...
ax.set_ylabel('Difference in absolute risk of \n hospitalization within 12 months', fontsize = 28)
ax.set_xlabel('Patients (sorted)', fontsize = 36)
ax.legend(fontsize = 36)
ax.hlines(y=0, xmin=0, xmax=len(z), linewidth=4, color='k', linestyle='--')
plt.show()
...
#%%
# Concordance-statistics for benefit (TE MATCHING)
x_test['y'] = y_test
X_t = x_test[x_test['T'] == True].reset_index(drop=True)
X_c = x_test[x_test['T'] == False].reset_index(drop=True)
