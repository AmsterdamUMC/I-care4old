...
outcome = 'K6A_target' # unsteady gait
...
'painmed_temp',  
'atc_analgesics', 
'MMC_cardio_respiratory',
'MMC_alzheimer_dementia',
'MMC_non_alzheimer_dementia',
'MMC_psychiatric',
'MMC_cerebrovascular',
'psychotropic_temp',
'atc_hypnotics_and_sedatives',
'CPS_3',
'atc_antidepressants_SSRI_v2',
'J1E',
'problems_because_of_drinking'
]
...
'Pain medication', 
'Analgesics',
'MMC Cardio_respiratory',
'MMC Alzheimer dementia',
'MMC Non-alzheimer dementia',
'MMC Psychiatric',
'MMC Cerebrovascular',
...
y_train_all = X_orig_train['K6A_target'].astype(float)
x_train_all = X_orig_train
y_test = X_orig_test['K6A_target'].astype(float)
x_test = X_orig_test

# Normalized data
#y_train_all = X_train['K6A'].astype(float)
#x_train_all = X_train
#y_test = X_test['K6A'].astype(float)
#x_test = X_test
...
# HUOM! Valitaan normalisoidut muuttujat (Jostakin syystä DML SHAP arvot ei toimi??)
...
for model_nro in range(len(models)):
    ...
    for train_idx, test_idx in kf.split(X_train_all):
        ...
        # Plot uplift curve
        if (model_nro==16): # talletaan valitun mallin käyrät
...
file_name = '/data/hyperparameters_anticholinergic_unsteady_gait'
...
#%%
# Mikä malli valitaan??
# Perustetaan valinta hyperparametrien etsintädataan
...
#%%
# Alustus dataframe ja listat ITE mallien tuloksien talennukselle
...
for model_nro in range(len(models)):
    ...
    for train_idx, test_idx in kf.split(X_train_all):
        ...
        # Plot uplift curve
        if (model_nro == 10): # Piirretään käyrät valitulle mallille
            uplift_disp=plot_uplift_curve(test['y'], treatment_effects, test['T'], perfect=False);
