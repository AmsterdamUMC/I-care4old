...
confounders_name = [
'ER visits, last 6 months (HILMO)',
'Age',
'Antipsychotics',
'Unsteady gait',
'Hours of physical activities in the last 3 days',
'DRS',
'ADL status has become worse',
'Primary modes of locomotion - Indoors',
'Neurological diagnosis',
'Problems because of drinking',
'Eating (6cat)',
'Locomotion in home (6cat)',
'ADL Long',
'Persistent anger with self or others (3 cat)',
'Number of drugs',
...
y_train_all = X_orig_train['ADLLF_decline_target'].astype(float)
x_train_all = X_orig_train
y_test = X_orig_test['ADLLF_decline_target'].astype(float)
x_test = X_orig_test

# Normalized data
#y_train_all = X_train['ADLLF_decline_target'].astype(float)
#x_train_all = X_train
#y_test = X_test['ADLLF_decline_target'].astype(float)
#x_test = X_test
...
file_name = '/data/hyperparameters_anticholinergic_adl_decline'
...
for model_nro in range(len(models)):
    ...
    for train_idx, test_idx in kf.split(X_train_all):
        ...
        # Plot uplift curve
        if (model_nro == 4): # Piirretään käyrät valitulle mallille
            uplift_disp=plot_uplift_curve(test['y'], treatment_effects, test['T'], perfect=False);
