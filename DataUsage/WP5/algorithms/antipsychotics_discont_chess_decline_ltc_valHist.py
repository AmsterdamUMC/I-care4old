...
outcome = 'SCHESS_ltc_decline_target'
...
y_train_all = X_orig_train['SCHESS_ltc_decline_target'].astype(float)
x_train_all = X_orig_train
y_test = X_orig_test['SCHESS_ltc_decline_target'].astype(float)
x_test = X_orig_test

# Normalized data
#y_train_all = X_train['SCHESS_ltc_decline_target'].astype(float)
#x_train_all = X_train
#y_test = X_test['SCHESS_ltc_decline_target'].astype(float)
#x_test = X_test
...
file_name = '/data/hyperparameters_antipsychotics_disc_chess_decline'
...
model_ate = []
model_auuc = []model_c_for_benefit = []
match_error = []
for k in range (len(auuc_models)):
    model_ate.append(str(np.round(np.mean(ate_all_models[k]),5)) + ' (' + str(np.round(np.min(ate_all_models[k]),4)) + ',' + str(np.round(np.max(ate_all_models[k]),4))+')')
    model_auuc.append(str(np.round(np.mean(auuc_models[k]),5)) + ' (' + str(np.round(np.min(auuc_models[k]),4)) + ',' + str(np.round(np.max(auuc_models[k]),4))+')')
